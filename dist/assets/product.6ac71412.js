import { g as o, s as d, a as e, b as i, P as n } from "./main.9e82ff00.js";
/* empty css              */ class l {
  constructor(a, t) {
  ...
  addProductToCart(a) {
    let t = o("so-cart");
    t === null && (t = []), t.push(a), d("so-cart", t);
  }
  async addToCartHandler(a) {
    this.play();
    const t = await this.dataSource.findProductById(a.target.dataset.id);
    this.addProductToCart(t), e();
  }
  async removeFromCartHandler(a) {
    const t = await this.dataSource.findProductById(a.target.dataset.id);
    removeProductFromCart(t), e();
  }
  renderProductDetails() {
    let a = Math.trunc(this.calc_discount()),
  ...
  stop() {
    ...
    a.addEventListener("animationend", function () {
      a.classList.remove("cart-animate");
    });
  }
}
const u = new n(),
...
class m {
  ...
  async create() {
    (await this.fetchAlerts()).alerts.forEach((s) => {
      let r = document.createElement("p");
      (r.innerHTML = s.message),
        (r.style.backgroundColor = s.bgColor),
        (r.style.color = s.textColor),
